...
const CustomTooltip = ({ active, payload, label }) => {
  ...
  return (
    <div className="bg-popover border border-border rounded-xl px-3 py-2 text-xs space-y-0.5">
      <p className="text-muted-foreground mb-1">{label}</p>
      {payload.map((p, i) => (
        <p key={i} className={p.dataKey === "refunds" ? "text-destructive font-semibold" : "text-foreground font-semibold"}>
...
const StatCard = ({ icon: Icon, label, value, subLabel }) => (
  <div className="bg-card border border-border rounded-2xl px-5 py-4 flex-1 min-w-[220px]">
    ...
    {subLabel && <div className="text-xs mt-1 text-primary">{subLabel}</div>}
  </div>
);
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-4">
      <div className="flex justify-between items-center mb-3">
        <span className="text-sm text-foreground font-medium">{station.stationName}</span>
        <span className="text-xs text-muted-foreground">
          {station.occupiedCount} / {station.totalSlots} booked ·{" "}
          <span className="text-primary">{station.freeCount} free</span>
        </span>
      </div>
      {station.totalSlots > 0 ? (
        ...
        <p className="text-xs text-muted-foreground">No slots configured for this station.</p>
      )}
    </div>
  );
};
  ...
  return (
    <div className="min-h-screen bg-background overflow-hidden flex flex-col md:flex-row font-[Dm_Sans]">
      ...
      <header className="flex z-0 w-full md:hidden justify-between border border-b-border items-center px-4 py-3 bg-card">
        ...
        <button className="z-50 bg-secondary p-2 rounded-md text-secondary-foreground" onClick={toggleSidebar}>
      ...
      <div className="flex-1 h-screen overflow-y-auto custom-scroll px-4 py-5 md:px-8 md:py-7 w-full">
        ...
        <section className="bg-card border mb-6 border-border flex flex-col gap-4 rounded-3xl p-6 sm:flex-row sm:items-center sm:justify-between sm:p-7">
          <div className="min-w-0">
            <h1 className="text-xl text-foreground font-semibold">Payments</h1>
            ...
            <ChartCard
              ...
              headerRight={
                <select
                  ...
                  className="bg-input border border-border text-foreground text-xs px-3 py-1.5 rounded-lg outline-none"
                >
            ...
            >
              <div className="flex flex-col gap-3">
                {visibleStations.length > 0 ? (
                  ...
                  <p className="text-xs text-muted-foreground">No stations to show.</p>
                )}
              </div>
            </ChartCard>
            ...
            <ChartCard title="Revenue & refunds — last 7 days" subtitle="Combined across all stations">
              <div className="flex gap-4 mb-2 text-xs text-muted-foreground">
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-2.5 rounded-sm bg-primary inline-block" /> Revenue (₹)
                </span>
                <span className="flex items-center gap-1.5">
                  <span className="w-2.5 h-0.5 bg-destructive inline-block" /> Refunds (₹)
                </span>
              </div>
              <ResponsiveContainer width="100%" height={260}>
                <ComposedChart data={revenueLast7Days}>
                  ...
                  <Bar dataKey="revenue" fill="var(--color-primary)" radius={[6, 6, 0, 0]} barSize={22} />
                  <Line
                    type="monotone"
                    dataKey="refunds"
