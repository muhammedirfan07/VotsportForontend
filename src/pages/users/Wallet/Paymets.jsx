  ...
  return (
    <div className="min-h-screen bg-background p-3 sm:p-6">
      <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
        ...
        {/* ── Wallet Summary Card ── */}
        <div className="bg-gradient-to-br from-primary/50 to-primary/15 rounded-2xl p-5 sm:p-8 relative overflow-hidden border border-primary/20">
          <div className="absolute top-0 right-0 w-64 h-64 bg-primary/10 rounded-full blur-3xl pointer-events-none" />

          <div className="relative z-10">
            <div className="flex justify-between items-start mb-5 sm:mb-6 gap-3">
              <div className="min-w-0">
                <p className="text-muted-foreground text-xs sm:text-sm mb-1">Wallet Balance (paid)</p>
                <h1 className="text-4xl sm:text-5xl font-bold text-foreground truncate">
                  {isLoading ? "—" : `₹${(summary?.walletBalance || 0).toFixed(2)}`}
                </h1>
              </div>
              <div className="w-12 h-12 sm:w-14 sm:h-14 bg-primary/20 rounded-2xl flex items-center justify-center flex-shrink-0">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-card/60 rounded-xl px-3 py-3 sm:px-4 border border-primary/10">
                <div className="flex items-center gap-1.5 mb-1">
                  <ArrowUpCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-destructive flex-shrink-0" />
                  <span className="text-muted-foreground text-xs truncate">Total Spent</span>
                </div>
              ...
              <div className="bg-card/60 rounded-xl px-3 py-3 sm:px-4 border border-primary/10">
                <div className="flex items-center gap-1.5 mb-1">
                  <ArrowDownCircle className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-primary flex-shrink-0" />
                  <span className="text-muted-foreground text-xs truncate">Total Refunded</span>
                </div>
                <p className="text-foreground font-bold text-lg sm:text-xl">
        ...
        <div className="bg-card rounded-2xl p-4 sm:p-8 border border-border">
          ...
          <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4 mb-5">
            <div>
              <h2 className="text-xl sm:text-2xl font-bold text-foreground mb-1">Payment History</h2>
              <p className="text-muted-foreground text-sm">Recent transactions and refunds</p>
            </div>
            ...
            <div className="relative w-full sm:w-auto sm:flex-shrink-0">
              <button
                ...
                className="w-full sm:w-auto bg-secondary hover:bg-muted text-foreground px-4 py-2.5 rounded-xl flex items-center justify-between gap-3 border border-border transition-all sm:min-w-[170px]"
              >
              ...
              {showFilterDropdown && (
                <div className="absolute top-full mt-2 left-0 right-0 sm:right-0 sm:left-auto sm:w-48 bg-popover border border-border rounded-xl overflow-hidden z-20 shadow-xl">
                  {timeFilters.map((f) => (
                    <button
                      key={f.value}
          ...
          <div className="flex gap-1.5 mb-5 bg-secondary p-1 rounded-xl w-fit max-w-full overflow-x-auto">
            {[
              { key: "all",      label: "All" },
          ...
          {selectedFilter !== "all" && (
            <div className="flex items-center gap-2 mb-4">
              ...
              <span className="text-xs bg-primary/10 text-primary border border-primary/30 px-2.5 py-0.5 rounded-full">
                {timeFilters.find((f) => f.value === selectedFilter)?.label}
              </span>
              <button
          ...
          <div className="space-y-3 max-h-[480px] sm:max-h-[540px] overflow-y-auto pr-1 custom-scroll">
            {isLoading ? (
              ...
              displayList.map((txn) => (
                <div
                  key={txn._id}
                  className="bg-secondary rounded-xl p-3 sm:p-4 border border-border hover:border-primary/40 transition-all flex items-center gap-3 sm:gap-4"
                >
                  {/* Icon */}
                  <div
                    className={`w-10 h-10 sm:w-11 sm:h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${
                      txn.type === "refund" ? "bg-primary/20" : "bg-destructive/10"
                    }`}
                  >
                    {txn.type === "refund"
                      ? <ArrowDownCircle className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
                      : <ArrowUpCircle  className="w-4 h-4 sm:w-5 sm:h-5 text-destructive" />}
                  </div>

                  {/* Info — truncates on small screens */}
                  <div className="flex-1 min-w-0">
                    <p className="text-foreground font-medium text-sm truncate">{txn.label}</p>
                    {txn.sub && (
                      <p className="text-muted-foreground text-xs truncate">{txn.sub}</p>
                    )}
                    {txn.detail && (
                      <p className="text-muted-foreground/70 text-xs mt-0.5 truncate hidden sm:block">
                        {txn.detail}
                      </p>
                    )}
                  ...
                  <div className="text-right flex-shrink-0">
                    <p
                      className={`font-bold text-base sm:text-lg ${
                        txn.type === "refund" ? "text-primary" : "text-destructive"
                      }`}
                    >
